...
function openProductDetail(categoryId, productId) {
  if (!window.productsData) return;
  const category = window.productsData.find(c => c.id === categoryId);
  if (!category) return;
  ...
  modalBody.innerHTML = `
    <div class="product-detail-modal-content slideInUp">
      <div class="product-detail-main">
        <img src="${product.image || category.icon}" alt="${product.name}" class="product-detail-image" />
      ...
      <form id="insurance-calc-form" class="insurance-calc-form">
        <h4><i class="fas fa-calculator"></i> 보험료 계산기</h4>
        <div class="form-group">
          <label>가입자 나이</label>
          <input type="number" name="age" min="18" max="80" required placeholder="예: 30" />
        </div>
        <div class="form-group">
          <label>가입 금액(만원)</label>
          <input type="number" name="amount" min="100" max="10000" required placeholder="예: 1000" />
        </div>
        <button type="submit" class="btn btn-primary">보험료 계산</button>
        <div class="calc-result" id="calc-result" style="margin-top:1rem;"></div>
      </form>
    </div>
  `;
  ...
  if (form) {
    form.onsubmit = function(e) {
      ...
      // 샘플 계산 로직 (실제 로직은 상품별로 다르게 확장 가능)
      let base = 10000;
      if (age < 30) base -= 1000;
      if (amount > 2000) base += (amount - 2000) * 2;
      const result = base + age * 10;
      document.getElementById('calc-result').innerHTML = `<strong>예상 보험료: <span style='color:#6366f1'>${result.toLocaleString()}원</span></strong>`;
    };
  }
}
